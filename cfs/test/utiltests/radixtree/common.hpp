...
// this file contains some common code for all tests to reduce the number of
// copypaste lines
...
template <typename _RAIter> bool is_unique(_RAIter begin, _RAIter end) {
  std::sort(begin, end);
  _RAIter it = std::adjacent_find(begin, end);
  return it == end;
}

template <typename T, size_t N> std::vector<T> make_vector(const T (&data)[N]) {
  return std::vector<T>(data, data + N);
}

std::vector<std::string> get_unique_keys() {
  const std::string unique_keys_strings[] = {"a",   "b",   "ab",  "ba",  "aa",
                                             "bb",  "aaa", "aab", "aba", "baa",
                                             "bba", "bab", "abb", "bbb"};
