...
void check_nonexistent_prefixes(tree_t &tree) {
  ...
  const std::string never_found_strings[] = {"abcdfe", "abcdefe", "abe",
                                             "cc",     "abcdec",  "bcdefc"};
  std::vector<std::string> should_never_be_found =
      make_vector(never_found_strings);
...
TEST(prefix_match, complex_tree) {
...

  {
    ...
    for (tree_t::iterator it = tree.begin(); it != tree.end(); ++it) {
      ...
      ASSERT_NE(map_found.end(), map_found.find(it->first))
          << "there is no such key in found";
  ...
  }
  {
    ...
    prefixes_t prefixes;
    {
      ...
      // iterate over each key in tree, make prefixes from it: prefix =
      // key[0..N], N <- [0 .. lenght key]
      for (tree_t::iterator it = tree.begin(); it != tree.end(); ++it) {
        ...
        for (size_t i = 0; i < key.size(); i++) {
          ...
          vector_found_t vec;
          for (tree_t::iterator each_it = tree.begin(); each_it != tree.end();
               ++each_it) {
    ...
    for (prefixes_t::const_iterator prefix_it = prefixes.begin();
         prefix_it != prefixes.end(); ++prefix_it) {
